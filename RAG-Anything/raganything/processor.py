...
class ProcessorMixin:
    ...
    async def _get_cached_result(
        ...
        try:
            ...
            self.logger.warning(f"Parse cache not properly initialized: {e}")
            self.logger.debug(f"Parse cache type: {type(self.parse_cache)}")
            self.logger.debug(f"Parse cache value: {self.parse_cache}")
            return None
